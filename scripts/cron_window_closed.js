...
function processUsersPicks(user, callback) {
	db.Pick.find({where:{ UserId: user.id, week: CONSTANTS.WEEK_OF_SEASON }}).success(function(pick) {
		if (pick) {
			...
			db.Pick.find({where:{ UserId: user.id, week: CONSTANTS.WEEK_OF_SEASON - 1}}).success(function(last_week_pick) {
...
function main() {
	...
	db.User.findAll({where:{status:'active'}}).success(function(users) {
		...
		users.forEach(function(user) {
			...
			processUsersPicks(user, function(err) {
				if (err)
